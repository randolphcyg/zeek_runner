...
global auth_managers: set[addr] = { 172.16.11.1 } &redef;  # 可信任的scp连接ip
...
event connection_state_remove(c: connection) {
    ...
    local byterate: double = |c$duration| > 0 ? c$conn$orig_ip_bytes / |c$duration| : 0;

    ##! If an SSH session has a ton of bytes, takes too long, or looks too big, it probably is - send an alert and log it!
    if ((c$duration > max_ssh_duration) || (byterate > max_ssh_rate) || (c$conn$orig_ip_bytes > max_ssh_size)) {
        ##! Print out alert to terminal
        print fmt("[WARNING] Potential unauth SCP detected (ATT&CK T1105)");
        print fmt("Time: %s TX: %s RX: %s Bytes: %s Duration: %s Connection: %s",
                  strftime("%Y/%m/%d %H:%M:%S", c$conn$ts),
                  c$id$orig_h, c$id$resp_h, c$conn$orig_ip_bytes, c$duration, c$uid);

        ##! And capture for posterity in notice.log
        NOTICE([
            $note = Weird::Activity,  # 使用预定义的日志类型
